...
idd = 199
idd_text = "198 (new model) with 1024,1024, constant biases, longer grind time"
...
def trainer(model, data,parameters, validatedata,validateparams,epochs=1, lr=1e-3, batch_size=10, test_num=0, weight_decay=None):
    ...
    scheduler = optim.lr_scheduler.MultiStepLR(
        optimizer,
        milestones=[35000, 40000],  # change after N and N+M steps
...
class main_net(nn.Module):
    def __init__(self, hidden_dims=[1024],output_length=1000, conv_channels=16):
        ...
        self.pointwise.apply(init_weights_constant)
        self.dilated_block.apply(init_weights_constant)

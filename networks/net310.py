...
def thisnet():
    ...
    conv_channels = 64
    output_length=1001
    model = main_net(output_length=output_length,hidden_dims=hidden_dims, conv_channels=conv_channels)
    return model
...
class main_net(nn.Module):
    ...
    def flux_diff(self, guess,target):
        ...
        i1 = rho
        i2 = rho*v
        i3 = 0.5*rho*v**2+p/(gamma-1)
        I = torch.stack([i1,i2,i3],dim=1)
        #add zero so the inverse of cumsum is centered difference.
        #zeros = torch.zeros(target.shape[0],target.shape[1],1, device=target.device, dtype=target.dtype)
        #I = torch.cat((zeros,I),dim=-1)
        I = torch.cumsum(I,dim=2)
        return self.l1(I,guess)
    def criterion(self,guess,target, initial=None):
        #L1 = self.l1(target,guess)
        if self.training:
            F = self.flux_diff(guess,target)
        else:
            F = self.l1(guess,target)
    ...
    def forward(self, x):
        ...
        if not self.training:
            ...
            rho = E1
            v = E2/rho
            p = (gamma-1)*(E3-0.5*E2**2/E1)
            x = torch.stack([rho,p,v],dim=1)

...
idd = 304
what = "fiducial for train on the hard ones"
...
def trainer(model, data,parameters, validatedata,validateparams,epochs=1, lr=1e-3, batch_size=10, test_num=0, weight_decay=None):
    ...
    scheduler = optim.lr_scheduler.MultiStepLR(
        optimizer,
        milestones=[35000, 40000,50000],  # change after N and N+M steps
    ...
    for epoch in range(epochs):
        ...
        if (epoch >= 10000 and epoch < 15000) and False:
            if nsub == 0:
                print('sort losses')
            ...
            subset = ars[nsub:nsub+batch_size]
            nsub += 1

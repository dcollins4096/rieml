...
idd = 404
what = "400 with remedial training, small model, run on only 3000 shock tubes."
time_data=True
...
def trainer(model, data,parameters, validatedata,validateparams,epochs=1, lr=1e-3, batch_size=10, test_num=0, weight_decay=None):
    ...
    nsub = 0
    loss_array = torch.ones(data.shape[0])
    for epoch in range(epochs):
        #train on the bad ones.
        if (epoch >= 60000 and epoch < 120000):
            if nsub == 0:
                print('sort losses')
                #train_loss = plot.compute_losses(model, data, parameters)
                args = torch.argsort(loss_array)

            #ars = torch.argsort(train_loss).flip(dims=[0])
            subset = args[batch_size*nsub:(nsub+1)*batch_size]
            nsub += 1
            if epoch % 300 == 0:
                nsub=0
